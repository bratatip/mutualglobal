...
<div class="w-full">
    @if (!empty($type) && $type === 'text')
        <input type="{{ $type }}"
            {{ $attributes->merge(['class' => 'text-xs text-gray-500 rounded-sm w-11/12 px-3 border-solid border-[#CCCCCC] focus:border-[#FFC451] focus:ring-0', 'id' => $name]) }}
            name="{{ $name }}"
            id="{{ $name }}"
    ...
    @elseif (!empty($type) && $type === 'date')
        <input type="{{ $type }}"
            {{ $attributes->merge(['class' => 'text-xs text-gray-500 rounded-sm px-3 border-solid border-[#CCCCCC] focus:border-[#FFC451] focus:ring-0', 'id' => $name]) }}
            name="{{ $name }}"
            id="{{ $name }}"
            placeholder="{{ $placeholder }}"
            @if ($disabled) disabled @endif
