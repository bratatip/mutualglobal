...
<div class="w-full">
    ...
    @elseif (!empty($type) && $type === 'number')
        <input type="{{ $type }}"
            {{ $attributes->merge(['class' => 'h-8 p-1 w-11/12 border-[#CCCCCC] border-1 border-solid focus:ring-0 focus:border-[#FFC451] focus:border-1 bg-white overflow-hidden text-gray-500 text-xs', 'id' => $name, 'value' => old($name)]) }}
            name="{{ $name }}"
            id="{{ $name }}"
            placeholder="{{ $placeholder }}"
            @if ($disabled) disabled @endif
