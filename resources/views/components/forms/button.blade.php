...
@if ($href)
    <a href="{{ $href }}"
        target="{{ $target }}"
        {{ $attributes->merge(['class' => 'inline px-6 py-2 mt-3 border border-solid rounded-2xl border-dark bg-gradient-to-r from-[#ef4444] to-[#15803d] bg-clip-text text-transparent text-xs ml-2 font-bold ' . $classes]) }}>
        {{ $text }}
    </a>
@else
    <button type="{{ $type }}"
